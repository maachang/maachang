...
//
// AIメモ:
// - 方針合意済みの割り切り仕様: 本モジュール内での個別サイズ上限チェックは行わず、
//   フレームワーク層 (conf/server.json の maxBodyLength、デフォルト 10MB) で
//   リクエスト全体のボディ長が事前に検証・保護される。
// - 同名フィールドが複数ファイルを持つケース(配列)には対応しない
//   (1フィールド1ファイル想定。同名パートが複数来た場合は最後のものが上書きで残る)。
// - $request().body() は常に Buffer で生バイナリを返す(GET以外)。
//   これがそのまま multipart パーサーの入力になる。
///////////////////////////////////////////////
(function () {
    'use strict';
